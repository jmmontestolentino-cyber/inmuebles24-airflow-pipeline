...
# 2. Función Principal que ejecutará Airflow
def ejecutar_scraping():
    # --- RUTAS DE LINUX DENTRO DE DOCKER ---
    # Airflow mapea la carpeta 'dags' a '/opt/airflow/dags' internamente
    filename = ""
    ...
    def iniciar_navegador_linux():
        ...
        # 1. Forzar un User-Agent de Windows/Chrome moderno
        ...
        # 2. Desactivar el webdriver flag explícitamente (por si acaso)
    ...
    def scrap_store(lista_urls, herramienta, nombre_pagina, tipo_operacion, fecha, page):
        # Guardaremos el CSV también en la carpeta dags para que puedas descargarlo fácil
            ...
            for l in range(1, numero_indice + 1):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        page.get_screenshot(path=f'/opt/airflow/dags/debug_inmuebles24_{i}_{l}.png')
    ...
    try:
        ...
        page.quit() # Super importante apagar el navegador al terminar para liberar memoria RAM
